...
def test_rpc_reverts_between_runs(SMTestBase):
    account_a = boa.env.generate_address()
    account_b = boa.env.generate_address()
    boa.env.set_balance(account_a, 10**18)

    class StateMachine(SMTestBase):
        def initialize_one(self):
            block = boa.env.vm.get_block()
            assert block.header.block_number == 1
            with boa.env.prank(account_a):
                boa.env.execute_code(
