...
def _address_strategy(length: Optional[int] = 100) -> SearchStrategy:
    ...
    # inject env addresses, ultimately, will sample length + 1 + len(boa.env._contracts) addresses
    accounts += [boa.env.eoa]
    accounts += [contract.address for contract in boa.env._contracts.values()]
    # TODO: add all addresses generated by env through env.generate_address()
    return _DeferredStrategyRepr(
        lambda: st.sampled_from(list(accounts)), "accounts"
    )
