...
def _address_strategy(
    length: Optional[int] = 100, env: Optional[EnvType] = None
) -> SearchStrategy:
    if env is None:
        env = boa.env
    ...
    st_random_account = st.binary(min_length=20, max_length=20).map(Address)

    return _DeferredStrategyRepr(
        lambda: st.one_of(st_env_account, st_random_account), "accounts"
    )
